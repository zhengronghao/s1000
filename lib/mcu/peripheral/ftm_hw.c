...
uint8 LPLD_FTM_Init(FTM_InitTypeDef ftm_init_structure)
{
	uint8 result, i;
	//参数检查
	ASSERT(ftm_init_structure.FTM_Mode & (FTM_MODE_PWM | FTM_MODE_IC | FTM_MODE_QD | FTM_MODE_DEC));	//判断模式选择
 ...
 *    duty--PWM输出占空比
 *      |__0~FTM_PWM_DUTY_MAX--占空比0.00%~100.00%
 *    pin--FTMx通道对应的引脚
 *      FTM0
 *       FTM_Ch0-PTA3(ALT3)\PTC1(ALT4)
 *       FTM_Ch1-PTA4(ALT3)\PTC2(ALT4)
 *       FTM_Ch2-PTA5(ALT3)\PTC3(ALT4)\PTC5(ALT7)
 *       FTM_Ch3-PTC4(ALT4)
 *       FTM_Ch4-PTB12(ALT4)\PTD4(ALT4)
 *       FTM_Ch5-PTA0(ALT3)\PTB13(ALT4)\PTD5(ALT4)
 *       FTM_Ch6-PTA1(ALT3)\PTD6(ALT4)
 *       FTM_Ch7-PTA2(ALT3)\PTD7(ALT4)
 *      FTM1
 *       FTM_Ch0-PTB12(ALT3)\PTA12(ALT3)\PTB0(ALT3)
 *       FTM_Ch1-PTB13(ALT3)\PTA13(ALT3)\PTB1(ALT3)
 *      FTM2
 *       FTM_Ch0-PTB18(ALT3)
 *       FTM_Ch1-PTB19(ALT3)
 *    align--脉冲对齐方式
 ...
 *    1--配置成功
 */
uint8 LPLD_FTM_PWM_Enable(FTM_Type * ftmx, FtmChnEnum_Type chn, uint32 duty, PortPinsEnum_Type pin,
	uint8 align)
{
	uint32 cv;
	vuint32 mod;
	...
	//参数检查
	ASSERT(duty <= FTM_PWM_DUTY_MAX);				//判断占空比
	...
	{
		duty = FTM_PWM_DUTY_MAX - duty;
	}

	//占空比 = (CnV-CNTIN)/(MOD-CNTIN+1)
	mod = ftmx->MOD;
	cv = (duty * (mod - 0 + 1) + 0) / FTM_PWM_DUTY_MAX;
 ...
 *    1--配置成功
 */
uint8 LPLD_FTM_IC_Enable(FTM_Type * ftmx, FtmChnEnum_Type chn, PortPinsEnum_Type pin,
	uint8 capture_edge)
{
	if (!LPLD_FTM_PinInit(ftmx, chn, pin))
		return 0;
 ...
 *
 */
boolean LPLD_FTM_IsTOF(FTM_Type * ftmx)
{
 ...
 *
 */
void LPLD_FTM_ClearTOF(FTM_Type * ftmx)
{
 ...
 *
 */
 boolean LPLD_FTM_IsCHnF(FTM_Type * ftmx, FtmChnEnum_Type chn)
{
 ...
 *
 */
 void LPLD_FTM_ClearCHnF(FTM_Type * ftmx, FtmChnEnum_Type chn)
{
 ...
 *
 */
 uint16 LPLD_FTM_GetChVal(FTM_Type * ftmx, FtmChnEnum_Type chn)
{
 ...
 *
 */
 uint8 LPLD_FTM_GetClkDiv(FTM_Type * ftmx)
{
 ...
 *
 */
 uint16 LPLD_FTM_GetCounter(FTM_Type * ftmx)
{
 ...
 *
 */
 void LPLD_FTM_ClearCounter(FTM_Type * ftmx)
{
	ftmx->CNT = 0;						//清空FTMx计数器值
}


void LPLD_FTM_Enable_FTM(FTM_Type * ftmx)
{
	ftmx->SC |= FTM_SC_TOIE_MASK;             //使能计数溢出中断
}

void LPLD_FTM_Disable_FTM(FTM_Type * ftmx)
{
	ftmx->SC &= ~FTM_SC_TOIE_MASK;             //失能计数溢出中断
}
...
//    TRACE("\r\n pwm_init[%d-%d]",freq,dt_val);
	...
	// 设置PWM周期及占空比   FTM时钟源:50MHz,周期20ns, 分频系数:1
	//    PWM周期 = (MOD-CNTIN+1)*FTM时钟周期 :
	//    MOD = PWM周期/FTM时钟周期-1+CNTIN = MOD
	// 配置FTM计数初始值
	ftmx->CNT = 0;
	ftmx->CNTIN = 0;
	// 配置FTM计数MOD值
    ftmx->MOD = (periph_clk_khz*1000)/freq-1;

	ftmx->DEADTIME = FTM_DEADTIME_DTPS(dt_div) | FTM_DEADTIME_DTVAL(dt_val);
	ftmx->COMBINE = dt_en;				//使能死区

	// 配置FTM控制寄存器
	// 禁用中断, 加计数模式, 时钟源:50MHz,周期20ns, 分频系数:0
	ftmx->SC = FTM_SC_CLKS(1) | FTM_SC_PS(0);
//    TRACE("\r\n pwm:%d-%d",freq,ftmx->MOD);
	return 1;
}
...
static uint8 LPLD_FTM_IC_Init(FTM_InitTypeDef ftm_init_structure)
{
 ...
 //   ftmx->MODE |= FTM_MODE_FTMEN_MASK;	//FTM0EN=1  //9.24增加
	...
	if (ftmx == FTM0)
	{
		...
		case FTM_Ch2:
			if (pin == PTA5)
				...
				PORTC->PCR[5] = PORTC->PCR[5] & ~PORT_PCR_MUX_MASK | PORT_PCR_MUX(7);
			else
				return 0;
			break;
		case FTM_Ch3:
		...
		case FTM_Ch4:
			if (pin == PTB12)
				PORTB->PCR[12] = PORTB->PCR[12] & ~PORT_PCR_MUX_MASK | PORT_PCR_MUX(4);
			else if (pin == PTD4)
		...
		case FTM_Ch5:
			if (pin == PTA0)
				...
				PORTB->PCR[13] = PORTB->PCR[13] & ~PORT_PCR_MUX_MASK | PORT_PCR_MUX(4);
			else if (pin == PTD5)
...
//			break;
    ...
    else if (ftmx == FTM3)
	{
		...
		case FTM_Ch3:
			if (pin == PTD3)
				PORTD->PCR[3] = PORTC->PCR[3] & ~PORT_PCR_MUX_MASK | PORT_PCR_MUX(4);
			else
				return 0;
			break;
		case FTM_Ch4:
