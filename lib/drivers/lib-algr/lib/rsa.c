...
static	unsigned int PstCycle = 0;

int EXPMODPerfDump()
{
    TRACE( "\n EXPMOD perf, cpu cycle %d\n\n", PstCycle - PreCycle);
}
*/
                 ...
                 unsigned char *output )
{
     ...
     * T2 = input ^ dQ mod Q
     */
  //  PreCycle =  *((unsigned int *)0xE0001004);
    MPI_CHK( mpi_exp_mod( &T1, &T, &ctx->DP, &ctx->P, &ctx->RP ) );
    MPI_CHK( mpi_exp_mod( &T2, &T, &ctx->DQ, &ctx->Q, &ctx->RQ ) );
  //  PstCycle =  *((unsigned int *)0xE0001004);
...
int rsa_pkcs1_encrypt_simpleRNG( rsa_context *ctx,
        ...
        if( mode == RSA_PUBLIC )
        {
            *p++ = RSA_CRYPT;
            ...
            while( nb_pad-- > 0 )
            {
                ...
                do {
                    *p = hardclock(); //??? generate random number 
...
#else //set padding word by word, so have pow(255, 3) possibility            

            //move p to word aligned addr
            while( ((unsigned int)p % 4) && (nb_pad > 0) )
            {
                *p = hardclock();
                if(*p == 0)
                {
            ...
            wordNum = nb_pad >> 2;
            restByteNum = nb_pad & 3;
            pInt = (unsigned int *)p;
            //TRACE("wordNum %d, restByteNum %d\n", wordNum, restByteNum);	
            //set word each time
            while( wordNum-- > 0 )
            {
                randNum = hardclock(); //high byte is 0
