...
//                    }
                ret = s_emvGuiWupa(buf);
//                ucVal = 0;
//                as3911ReadRegister(AS3911_REG_AM_MOD_DEPTH_CONF, &ucVal);
//                Dprintk("\r\n reg 24h:%x \r\n",ucVal);
//                ucVal = 0;
            ...
            while ( 1 ) {
                ...
                if ( !ret ) {
                    ...
                    if ( !ret ) {
                        ...
                        guiDebugFlg = 1;
                        emvInitLayer4(&picc);
                        ret = emvTransceiveApdu(buf, 5, &buf[5], 16, &i);
                        guiDebugFlg = 0;
                        Dprintk("\r\n\r\n --------ret  :%d   ",ret);
                        if ( !ret ) {
                            DISPBUF(&buf[5],i,0);
...
//            Dprintk("\r\n\r\n stop:%x  %d",emvStopRequestReceived(),TRUE);
            ...
            i = 0;
            as3911WaitForInterruptTimed(AS3911_IRQ_MASK_DCT, 20, &i);
            Dprintk("\r\n\r\n interrupt flag  :%x",i);
...
//            do_program_download(DOWNLOAD_MODE_AUTO);
        ...
        case 23:
            ...
            DISPBUF(EG_mifs_tWorkInfo.aucCmdBuf, 6, 0);
...
//    if ( id != 0 ) {
        as3911WriteRegister(AS3911_REG_VSS_REGULATOR_CONF, gas3911paramtable[id].reg2Ah);
        as3911WriteRegister(AS3911_REG_RFO_AM_OFF_LEVEL, gas3911paramtable[id].reg27h);
        as3911WriteRegister(AS3911_REG_ANT_CAL_CONF, gas3911paramtable[id].reg21h);
        ...
        as3911ReadRegister(AS3911_REG_VSS_REGULATOR_CONF, &ucVal);
        Dprintk("\r\n --2Ah: %02x",ucVal);
        as3911ReadRegister(AS3911_REG_RFO_AM_OFF_LEVEL, &ucVal);
        Dprintk("\r\n --27h: %02x",ucVal);
        as3911ReadRegister(AS3911_REG_ANT_CAL_CONF, &ucVal);
        Dprintk("\r\n --21h: %02x",ucVal);
...
int test_as3911_emv(int mode)
{
	...
	GPIO_InitStructure.GPIO_Pin =  Debug_SIGNAL_Pin|Debug_SIGNAL1_Pin;
	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP;
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;
	GPIO_Init(Debug_SIGNAL_Port, &GPIO_InitStructure);
	GPIO_Init(Debug_SIGNAL1_Port, &GPIO_InitStructure);
	GPIO_ResetBits(Debug_SIGNAL_Port,Debug_SIGNAL_Pin);
	GPIO_ResetBits(Debug_SIGNAL1_Port,Debug_SIGNAL1_Pin);
    SETSIGNAL_L();
    SETSIGNAL1_L();
#endif
...
//                applProcessCmd(buf, sizeof(gas3911paramtable[i].regtable), buf, &ucVal);
                ...
                test_loadas3911regtable(i, &gas3911paramtable[i].regtable[14]);
...
//            test_loadas3911regtable(i, NULL);
        ...
        case 5:
            ...
            Dprintk("\r\n\r\n ----y table----");
            as3911WriteRegister(AS3911_REG_AM_MOD_DEPTH_CONF, 0x1E);//type b调制深度11%
            for ( i=0 ; i<6 ; i++  ) {
                if ( InkeyCount(0) == 1 ) {
                    break;
                }
...
//            as3911ExecuteCommand(AS3911_CMD_SET_DEFAULT);
//            s_DelayMs(100);
//			as3911WriteRegister(AS3911_REG_AM_MOD_DEPTH_CONF, 0x1E);
...
//                vDispBufKey(guiDebugS3,gucDebugBuf);
            ...
            break;
        case 15:
            Dprintk("\r\n\r\n -----save the table 0 to the flash");
            CLRBUF(gcDebugBuf);
            j = Write3911_new(28, (uchar *)&gas3911paramtable[0]);
            Dprintk("\r\n\r\n -----save result:%d",j);
            break;
        case 16:
//            SETSIGNAL_H();
...
//                DISPBUF(picc.uid, picc.uidLength, 0);
                ...
                ret = emvTransceiveApdu(buf, 5, &buf[5], 16, (int *)&key);
                if ( !ret ) {
                    DISPBUF(&buf[5],(int)key,0);
                    //            sys_beep();
                    succ++;
                }
...
//            as3911_StartTimer(AS3911_TIMER);
        ...
        case 98:
            ...
            ucVal = InkeyHex(0);
//            as3911ModifyRegister(AS3911_REG_AM_MOD_DEPTH_CONF, 0x7E, 0x1E);
            as3911ModifyRegister(AS3911_REG_AM_MOD_DEPTH_CONF, 0x7E, ucVal);
            as3911ClearInterrupts(AS3911_IRQ_MASK_DCT);
            as3911EnableInterrupts(AS3911_IRQ_MASK_DCT);
            as3911ExecuteCommand(AS3911_CMD_CALIBRATE_MODULATION);
            as3911WaitForInterruptTimed(AS3911_IRQ_MASK_DCT, 20, (ulong *)&j);
