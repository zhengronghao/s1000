...
{
    TRACE(" LLWU Reset pin enabled as wakeup source from Low power modes \n");
...
void llwu_configure(unsigned int pin_en, unsigned char rise_fall, unsigned char module_en )
{
    ...
    if( pin_en & 0x0001)
    {
        ...
        TRACE(" LLWU configured pins PTE1/UART1_RX/I2C1_SCL /SPI1_SIN to be an LLWU wakeup source \n");
        LLWU_F1 |= LLWU_F1_WUF0_MASK;   // write one to clear the flag
    }
    if( pin_en & 0x0002)
    {
        temp |= LLWU_PE1_WUPE1(rise_fall);
        TRACE(" LLWU configured pins PTE2/SPI1_SCK/SDHC0_DCLK to be an LLWU wakeup source \n");
        LLWU_F1 |= LLWU_F1_WUF1_MASK;   // write one to clear the flag
    }
    if( pin_en & 0x0004)
    {
        temp |= LLWU_PE1_WUPE2(rise_fall);
        TRACE(" LLWU configured pins PTE4/SPI1_PCS0/SDHC0_D3 to be an LLWU wakeup source \n");
        LLWU_F1 |= LLWU_F1_WUF2_MASK;   // write one to clear the flag
    }
    if( pin_en & 0x0008)
    {
        temp |= LLWU_PE1_WUPE3(rise_fall);
        TRACE(" LLWU configured pins PTA4/FTM0_CH1/NMI/EZP_CS to be an LLWU wakeup source \n");
    ...
    if( pin_en & 0x0010)
    {
        ...
        TRACE(" LLWU configured pins PTA13/FTM1_CH1 /FTM1_QD_PHB to be an LLWU wakeup source \n");
        LLWU_F1 |= LLWU_F1_WUF4_MASK;   // write one to clear the flag
    }
    if( pin_en & 0x0020)
    {
        temp |= LLWU_PE2_WUPE5(rise_fall);
        TRACE(" LLWU configured pins PTB0/I2C0_SCL/FTM1_CH0/FTM1_QD_PHA to be an LLWU wakeup source \n");
        LLWU_F1 |= LLWU_F1_WUF5_MASK;   // write one to clear the flag
    }
    if( pin_en & 0x0040)
    {
        temp |= LLWU_PE2_WUPE6(rise_fall);
        TRACE(" LLWU configured pins PTC1/UART1_RTS/FTM0_CH0 to be an LLWU wakeup source \n");
        LLWU_F1 |= LLWU_F1_WUF6_MASK;   // write one to clear the flag
    }
    if( pin_en & 0x0080)
    {
        temp |= LLWU_PE2_WUPE7(rise_fall);
        TRACE(" LLWU configured pins PTC3/UART1_RX/FTM0_CH2 to be an LLWU wakeup source \n");
    ...
    if( pin_en & 0x0100)
    {
        ...
        TRACE(" LLWU configured pins PTC4/SPI0_PCS0/FTM0_CH3 to be an LLWU wakeup source \n");
        LLWU_F2 |= LLWU_F2_WUF8_MASK;   // write one to clear the flag
    }
    if( pin_en & 0x0200)
    {
        temp |= LLWU_PE3_WUPE9(rise_fall);
        TRACE(" LLWU configured pins PTC5/SPI0_SCK/I2S0_RXD0 to be an LLWU wakeup source \n");
        LLWU_F2 |= LLWU_F2_WUF9_MASK;   // write one to clear the flag
    }
    if( pin_en & 0x0400)
    {
        temp |= LLWU_PE3_WUPE10(rise_fall);
        TRACE(" LLWU configured pins PTC6/PDB0_EXTRG to be an LLWU wakeup source \n");
        LLWU_F2 |= LLWU_F2_WUF10_MASK;   // write one to clear the flag
    }
    if( pin_en & 0x0800)
    {
        temp |= LLWU_PE3_WUPE11(rise_fall);
        TRACE(" LLWU configured pins PTC11/I2S0_RXD1 to be an LLWU wakeup source \n");
    ...
    if( pin_en & 0x1000)
    {
        ...
        TRACE(" LLWU configured pins PTD0/SPI0_PCS0/UART2_RTS to be an LLWU wakeup source \n");
        LLWU_F2 |= LLWU_F2_WUF12_MASK;   // write one to clear the flag
    }
    if( pin_en & 0x2000)
    {
        temp |= LLWU_PE4_WUPE13(rise_fall);
        TRACE(" LLWU configured pins PTD2/UART2_RX to be an LLWU wakeup source \n");
        LLWU_F2 |= LLWU_F2_WUF13_MASK;   // write one to clear the flag
    }
    if( pin_en & 0x4000)
    {
        temp |= LLWU_PE4_WUPE14(rise_fall);
        TRACE(" LLWU configured pins PTD4/UART0_RTS/FTM0_CH4/EWM_IN to be an LLWU wakeup source \n");
        LLWU_F2 |= LLWU_F2_WUF14_MASK;   // write one to clear the flag
    }
    if( pin_en & 0x8000)
    {
        temp |= LLWU_PE4_WUPE15(rise_fall);
        TRACE(" LLWU configured pins PTD6/UART0_RX/FTM0_CH6/FTM0_FLT0 to be an LLWU wakeup source \n");
    ...
    TRACE("LLWU PE1   = 0x%02X,    ",   (LLWU_PE1)) ;
    TRACE("LLWU PE2   = 0x%02X\n",      (LLWU_PE2)) ;
    TRACE("LLWU PE3   = 0x%02X,    ",   (LLWU_PE3)) ;
    TRACE("LLWU PE4   = 0x%02X\n",      (LLWU_PE4)) ;
    TRACE("LLWU ME    = 0x%02X,    ",    (LLWU_ME)) ;
    TRACE("LLWU F1    = 0x%02X\n",       (LLWU_F1)) ;
    TRACE("LLWU F2    = 0x%02X,    ",    (LLWU_F2)) ;
    TRACE("LLWU F3    = 0x%02X\n",       (LLWU_F3)) ;
    TRACE("LLWU FILT1 = 0x%02X,    ", (LLWU_FILT1)) ;
    TRACE("LLWU FILT2 = 0x%02X\n",    (LLWU_FILT2)) ;
    TRACE("LLWU RST   = 0x%02X\n",      (LLWU_RST)) ;
      //function ends
}
...
*
*   PROCEDURE NAME:
*        LLWU_IRQHandler - Interrupt handler for LLWU
*
*******************************************************************************/
...
//  GPIOE_PSOR = 0x04000000;             // set Port E 26 indicate wakeup - set it in LLWU_IRQHandler 
